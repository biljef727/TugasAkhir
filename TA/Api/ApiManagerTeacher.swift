...
class ApiManagerTeacher {
    ...
    func fetchClassTeacher(userID: String, completion: @escaping(Result<([String]), Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) {
                do {
                    let decodedData = try JSONDecoder().decode([String: [String]].self, from: data)
                    guard let teacherClassID = decodedData["TeacherClass"] else {
                        throw NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid response format"])
                    }
                    completion(.success(teacherClassID))
                } catch {
                    print("Error parsing JSON: \(error)")
                    completion(.failure(error))
                }
