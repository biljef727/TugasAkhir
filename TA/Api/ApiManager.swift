...
//                    let _ = print("7")
    ...
    //                let _ = print("5")
                    do {
                        let decodedData = try JSONDecoder().decode([String: [String]].self, from: data)
                        guard let StudentNamesInsideClass = decodedData["StudentNameInsideClass"], let StudentIDInsideClass = decodedData["StudentIDInsideClass"] else {
    //                        let _ = print("6")
                            throw NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid response format"])
                        }
    //                    let _ = print("7")
                        completion(.success((StudentNamesInsideClass,StudentIDInsideClass)))
                    } catch {
                        print("Error parsing JSON: \(error)")
                    }
                } else {
